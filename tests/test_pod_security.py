...
class TestPodSecurityChecker:
    ...
    def test_detects_root_user(self):
        ...
        mock_pod = self._create_mock_pod(
            pod_name="test-pod", namespace="default", run_as_user=0
        )
    ...
    def test_detects_privileged_container(self):
        ...
        mock_pod = self._create_mock_pod(
            pod_name="privileged-pod", namespace="default", privileged=True
        )
    ...
    def test_detects_missing_security_context(self):
        ...
        mock_pod = self._create_mock_pod(
            pod_name="no-context-pod", namespace="default", security_context=None
        )
    ...
    def _create_mock_pod(
        ...
        if security_context:
            ...
            mock_container.security_context.allow_privilege_escalation = (
                allow_privilege_escalation
            )
            mock_container.security_context.run_as_non_root = (
                (run_as_user != 0) if run_as_user is not None else None
            )
