...
class TestRBACChecker:
    ...
    def test_detects_elevated_role_binding(self):
        ...
        assert len(findings) > 0
        assert any(
            "elevated" in f["title"].lower() or "admin" in f["title"].lower()
            for f in findings
        )
    ...
    def _create_mock_cluster_role_binding(
        self, binding_name, role_name, subject_name, subject_kind
    ):
    ...
    def _create_mock_role_binding(
        self, binding_name, namespace, role_name, subject_name, subject_kind
    ):
