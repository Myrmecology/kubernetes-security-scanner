...
class Reporter:
    ...
    def _print_severity_group(self, severity, findings):
        ...
        for i, finding in enumerate(findings, 1):
            print(
                f"{color}[{severity}] {finding.get('title', 'Unknown Issue')}{Style.RESET_ALL}"
            )
            ...
            if finding.get("recommendation"):
                print(
                    f"  {Fore.CYAN}💡 Recommendation: {finding['recommendation']}{Style.RESET_ALL}"
                )

            if finding.get("remediation"):
                print(
                    f"  {Fore.GREEN}🔧 Remediation: {finding['remediation']}{Style.RESET_ALL}"
                )
    ...
    def _print_summary(self):
        ...
        if critical_count > 0:
            print(
                f"{Fore.RED}🚨 Security Posture: CRITICAL - Immediate action required!{Style.RESET_ALL}"
            )
        elif high_count > 0:
            print(
                f"{Fore.YELLOW}⚠️  Security Posture: NEEDS ATTENTION - Address high-priority issues{Style.RESET_ALL}"
            )
        elif total > 0:
            print(
                f"{Fore.YELLOW}⚡ Security Posture: FAIR - Minor improvements recommended{Style.RESET_ALL}"
            )
        else:
            print(
                f"{Fore.GREEN}✅ Security Posture: EXCELLENT - No issues detected{Style.RESET_ALL}"
            )
